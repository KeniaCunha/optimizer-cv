...
async function baixarDescricaoVaga(link, browser) {
  try {
    ...
    try {
      const botaoVerMais = await page.evaluateHandle(() => {
        ...
        for (let botao of botoes) {
          const texto = (botao.textContent || botao.innerText || '').toLowerCase();
          if (texto.includes('ver mais') || 
              texto.includes('show more') || 
              texto.includes('see more') ||
              texto.includes('expandir') ||
              (botao.getAttribute('aria-label') && botao.getAttribute('aria-label').toLowerCase().includes('more'))) {
            return botao;
          }
        }
        return null;
    ...
    try {
      ...
      await page.waitForSelector('.show-more-less-html__markup, .jobs-description-content__text, [data-test-id="job-details-description"], .jobs-box__html-content', { 
        timeout: 10000 
      }).catch(() => {
    ...
    try {
      await page.evaluate(() => {
        ...
        const elementosDescricao = document.querySelectorAll('.show-more-less-html__markup, .jobs-description-content__text, [data-test-id="job-details-description"]');
        if (elementosDescricao.length > 0) {
          elementosDescricao[0].scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
      });
      await new Promise(resolve => setTimeout(resolve, 2000));
    } catch (e) {
    ...
    const descricao = await page.evaluate(() => {
      ...
      const elementosTestId = document.querySelectorAll('[data-test-id*="job"], [data-test-id*="description"], [data-test-id*="detail"]');
      ...
      const elementosComAtributos = document.querySelectorAll('[data-test-id], [class*="job"], [class*="description"]');
      for (let elemento of elementosComAtributos) {
        ...
        if (texto.length > 300 && 
          ...
          const palavrasChave = ['responsabilidade', 'requisito', 'experiência', 'habilidade', 
                                 'trabalho', 'equipe', 'desenvolvimento', 'projeto', 'tecnologia',
                                 'atribuição', 'desejável', 'diferencial', 'benefício', 'salário'];
      ...
      const secoesComParagrafos = document.querySelectorAll('section, div, article');
      for (let secao of secoesComParagrafos) {
        ...
        if (contadorParagrafos >= 3 && textoCompleto.length > 500) {
          ...
          if (!textoLimpo.toLowerCase().includes('linkedin') &&
              !textoLimpo.toLowerCase().includes('entrar') &&
              !textoLimpo.toLowerCase().includes('cadastre-se')) {
            return textoLimpo;
          }
      ...
      const palavrasChaveDescricao = ['responsabilidade', 'atribuição', 'requisito', 'desejável', 
                                      'diferencial', 'benefício', 'salário', 'remoto', 'presencial',
                                      'desenvolvimento', 'projeto', 'equipe', 'tecnologia'];
      ...
      for (let div of todosDivs) {
        ...
        if (texto.length > 400) {
          ...
          const temPalavrasChave = palavrasChaveDescricao.some(palavra => textoLower.includes(palavra));
          const naoTemMetadados = !textoLower.includes('nível de experiência') &&
      ...
      const todosElementos = Array.from(document.querySelectorAll('div, section, article, main'));
      ...
      for (let elemento of todosElementos) {
        ...
        if (texto.length > 500 &&
          ...
          const palavrasChave = ['responsabilidade', 'requisito', 'experiência', 'habilidade', 
                                 'atribuição', 'desejável', 'diferencial', 'benefício'];
          const temPalavrasChave = palavrasChave.some(palavra => 
            texto.toLowerCase().includes(palavra)
          );
      ...
      const elementosPrincipais = document.querySelectorAll('main, article, [role="main"]');
      ...
      const mainContent = document.querySelector('main, [role="main"], .jobs-search__job-details');
      if (mainContent) {
        const textoMain = (mainContent.innerText || mainContent.textContent || '').trim();
    ...
    if (descricao.trim().length < 300 || 
      ...
      console.log('  Descrição parece incompleta, tentando estratégia alternativa...');
      ...
      const descricaoAlternativa = await page.evaluate(() => {
        ...
        for (let elemento of todosElementos) {
          ...
          if (texto.length > 500) {
            const textoLower = texto.toLowerCase();
            const temPalavrasChave = textoLower.includes('responsabilidade') || 
            ...
            if (temPalavrasChave && naoTemMetadados && !textoLower.includes('linkedin')) {
              return texto;
            }
          }
        }
 ...
 */
async function gerarCurriculoATS(curriculoOriginal, descricaoVaga, promptPersonalizado = null) {
  try {
    ...
    const promptBase = promptPersonalizado || `Você é um especialista em recrutamento e otimização de currículos para sistemas ATS (Applicant Tracking System).
...
Retorne APENAS o currículo otimizado, sem explicações adicionais.`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'Você é um especialista em otimização de currículos para sistemas ATS. Sempre mantenha a veracidade das informações e otimize para corresponder às vagas.'
        },
        {
          role: 'user',
          content: promptBase
...
async function salvarCurriculoOtimizado(curriculoOtimizado, index, link) {
  try {
    ...
    // Conteúdo do arquivo
    const conteudo = `CURRÍCULO OTIMIZADO PARA VAGA ${index + 1}\n` +
                     `${'='.repeat(80)}\n\n` +
                     `LINK DA VAGA: ${link}\n` +
...
async function salvarDescricaoEmTxt(descricao, index, link) {
  try {
    ...
    // Conteúdo do arquivo
    const conteudo = `LINK DA VAGA: ${link}\n\n` +
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < vagas.length; i++) {
      ...
      if (descricao && descricao.length > 200 && !descricao.includes('Descrição não encontrada')) {
        try {
          ...
          const curriculoOtimizado = await gerarCurriculoATS(curriculo, descricao, promptPersonalizado);
          await salvarCurriculoOtimizado(curriculoOtimizado, i, link);
        } catch (error) {
          console.error(`  Erro ao gerar currículo otimizado: ${error.message}`);
          console.log('  Continuando para próxima vaga...');
        }
      } else {
        console.log('  Descrição muito curta ou não encontrada, pulando geração de currículo');
      }
    ...
    console.log(`Currículos otimizados salvos em: ${path.join(__dirname, 'curriculos_otimizados')}`);
